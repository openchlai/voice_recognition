...
def analyze_chunks(
    audio, sr, min_chunk_length_samples, max_chunk_length_samples,
    frame_length_samples, overlap_samples):
    """
...
def compute_dynamic_thresholds(audio, sr, frame_length_samples, segment_length_samples):
    ...
    for i in range(0, len(audio), segment_length_samples):
        ...
        energy = np.array([np.sum(segment[j * frame_length_samples:(
            j + 1) * frame_length_samples] ** 2) for j in range(n_frames)])
        
        # Calculate the Zero-Crossing Rate for each frame in the segment
        zcr = np.array([np.sum(librosa.zero_crossings(segment[j * frame_length_samples:(
            j + 1) * frame_length_samples], pad=False)) for j in range(n_frames)])
...
def split_audio(
    input_file, output_dir, output_format, min_chunk_length_ms, max_chunk_length_ms,
    frame_length_ms, sr, overlap_samples):
    """
...
def process_directory(
    ...
    for filename in os.listdir(source_dir):
        if filename.lower().endswith('.wav'):
            split_audio(
                os.path.join(source_dir, filename), output_dir, output_format,
                min_chunk_length_ms, max_chunk_length_ms, frame_length_ms, sr,
                overlap_samples)
